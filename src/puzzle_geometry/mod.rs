...
use num::{Matrix, Num, Vector, rotate_to};
use thiserror::Error;

use exact_trig::rotation_degree;
...
impl PuzzleGeometry {
    ...
    fn calc_permutation_group(&self) -> &(Arc<PermutationGroup>, BTreeSet<usize>) {
        self.perm_group_data.get_or_init(|| {
        ...
        self.ksolve_data.get_or_init(|| {
            ...
            for (name, perm) in group.generators() {
                ...
                for (orbit, ori_count) in orbits.iter().zip(orientation_counts.iter()) {
                    ...
                    for piece in orbit {
                        let first_one_goes_to = perm.mapping()[piece.stickers[0]];
    ...
    pub fn geometry(self) -> Result<PuzzleGeometry, PuzzleGeometryError> {
        ...
        for name in names {
            ...
            let possible_degrees = (2..=edges_that_might_map_together.len())
                .map(|d| {
                    (
                        rotation_degree(1, d).inner()[0][0].clone() * Num::from(2) + Num::from(1),
                        d,
                    )
                })
                .collect_vec();
            ...
            let mut matrices = edges_that_might_map_together
                ...
                .filter_map(|v| {
                    ...
                    let axis = Vector::new([[yz - zy, zx - xz, xy - yx]]);
            ...
            match matrices.find(|(matrix, _)| cloud.clone().try_symmetry(matrix)) {
                None | Some((_, 1)) => {
...
mod tests {
    ...
    use super::{
        ...
        num::{Num, Vector},
        point_compare, turn_compare, turn_names,
    };
    use internment::ArcIntern;
    use itertools::Itertools;
